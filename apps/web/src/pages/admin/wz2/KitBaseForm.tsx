...
export function KitBaseForm({ kitBaseId, gameId, onFinished }: Props) {
  ...
  const { data: kitBaseCategories } =
    trpc.admin.warzone2.kitBases.categories.useQuery();

  const [isEditingOption, setIsEditingOption] =
    useState<WarzoneTwoKitOption | null>(null);
  ...
  const { mutate: updateBase } =
    trpc.admin.warzone2.kitBases.update.useMutation();
  const { mutate: createBase } =
    trpc.admin.warzone2.kitBases.create.useMutation();
  const { mutate: deleteBase } =
    trpc.admin.warzone2.kitBases.delete.useMutation();
      ...
      >
        ...
        {isCreatingOption ? (
          ...
          <>
            ...
            {formValues.id
              ? Object.entries<WarzoneTwoKitOption[]>(getOptions()).map(
                  ([slotKey, options]) => (
                    <SubSection
                      action={
                        <Button
                    ...
                    >
                      <List>
                        {options
                          .sort(
                            (a, b) =>
                              Number(a.orderPlacement) -
                              Number(b.orderPlacement)
                          )
                          .map((option) => (
                            <List.Item
                              key={option.id}
