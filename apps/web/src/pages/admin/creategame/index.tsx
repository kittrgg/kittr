...
function Page() {
	...
	const { data: platforms, isLoading: isLoadingPlatforms } =
		trpc.games['list-platforms'].useQuery();
	...
	const handleInputChange = (
		...
		else if (inputKey === 'releaseDate')
			gameInputsCopy.releaseDate = new Date(e.value);
		// Gameinput strings
		...
		return (
			<Section title="Create Game">
				{Object.keys(gameInputs).map((input) => {
					if (input === 'active') {
						return (
							<Switch
								...
								size="lg"
								sx={{
									input: { backgroundColor: colors.light, border: 'none' },
					...
					if (input === 'releaseDate') {
						return (
							<>
								...
								<input
									onChange={({ target }) =>
										handleInputChange(target, 'releaseDate')
									}
									style={{
					...
					return (
						<TextInput
							key={input}
							label={input}
							onChange={({ target }) => handleInputChange(target, input as any)}
						/>
					);
				})}
				...
				<MultiSelect
					...
					label="Genres"
					onChange={(val) =>
						setGameInputs((prev) => ({ ...prev, genres: val }))
					}
				/>
				<MultiSelect
					...
					label="Platforms"
					onChange={(val) =>
						setGameInputs((prev) => ({ ...prev, platforms: val }))
					}
				/>
				{!isFormComplete() && (
					<Text color={colors.red}>
						Fill out form fully before submitting...
					</Text>
				)}
				...
				<Button
					...
					onClick={() => {
						addGame(
							{ ...gameInputs },
							{ onSuccess: () => console.log('Okay to clear...') },
						);
					}}
				>
					Add Game
