...
handler.post(async (req: NextApiRequest, res: NextApiResponse<NextServerPayload<Channel>>) => {
	...
	const { channelId, affiliateId, company, description, code, url } = JSON.parse(req.body)
...
handler.delete(async (req: NextApiRequest, res: NextApiResponse) => {
	try {
		...
		// Delete using the id, safer than code...but front-end will have to be updated to reflect this change.
		const result = await prisma.channel.update({
			where: { id: channelId },
			data: {
				profile: {
