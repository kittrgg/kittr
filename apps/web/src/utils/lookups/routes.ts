export const Routes = {
	CHANNEL: {
		...
		GAME: {
			...
			createPath: (channel: string, game: string, params?: string) =>
				`/channel/${channel}/${game}${params || ''}`,
			/** Output result: `/channel/:channel/:game/overlay` */
			createOverlayPath: (channel: string, game: string) =>
				`/channel/${channel}/${game}/overlay`,
		},
	},
	GAMES: {
		/** Output result: `/games` */
