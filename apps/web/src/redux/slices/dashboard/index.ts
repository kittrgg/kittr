...
export const dashboardSlice = createSlice({
	...
	reducers: {
		setActiveView: (
			state,
			action: { payload: { channelId: string; view: string } },
		) => {
		...
		handleTutorialAction: (
			...
			action: {
				payload: {
					condition: boolean;
					trueState: ReduxModalState;
					falseState: ReduxModalState;
				};
			},
		) => {
		...
		setChannelView: (
			state,
			action: { payload: { gameId: string; view: string } },
		) => {
		...
		updateBase: (
			state,
			action: { payload: WarzoneKitBase | WarzoneTwoKitBase },
		) => {
