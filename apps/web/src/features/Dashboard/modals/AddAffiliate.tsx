...
function AddAffiliateModal() {
	...
	const submit = () => {
		if (data?.id) {
			updateAffiliate({
				...
				data: { id: data?.id!, company, description, code, url },
			});
		} else {
			createAffiliate({
				channelId: channelData?.id!,
				data: { company, description, code, url },
			});
		}
	};
	...
	if (isCreatingAffiliate || isUpdatingAffiliate)
		return <Spinner width="24px" />;
	...
	return (
		<Modal backgroundClickToClose title="ADD AFFILIATE">
			...
			<Button
				...
				disabled={
					!company ||
					![description, code].some((elem) => elem.length) ||
