	...
	return (
		<Styled.Container>
			<Styled.HorizFlex>
				...
				<Styled.HeaderHelper>
					(Optional - Limit 20 characters)
				</Styled.HeaderHelper>
			</Styled.HorizFlex>
			<Styled.ParagraphHelper>
				This text will be displayed next to your weapon's default name.
			</Styled.ParagraphHelper>
			<TextInput
				inputStyles={{
					width: '50%',
				...
				onChange={(e) => {
					if (e.target.value.length === 0)
						return dispatch(updateCustomTitle(''));
					if (
						e.target.value.length > 20 ||
						!validator.matches(e.target.value, /^[a-z0-9 ]+$/i)
					) {
				...
				value={customTitle ?? ''}
			/>
			<Styled.ClosingParenthesis style={{ color: colors.lighter }}>
				)
			</Styled.ClosingParenthesis>
		</Styled.Container>
	);
}
