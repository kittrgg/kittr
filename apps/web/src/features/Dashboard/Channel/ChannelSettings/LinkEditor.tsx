...
function LinkEditor() {
	...
	useEffect(() => {
		...
		linkEdits.forEach((link) => {
			if (
				Object.entries(data?.links || {}).find(
					(ogLink) =>
						ogLink[0] === link.value && ogLink[1].value === link.value,
				)
			) {
	...
	const removeLink = (property: string) => {
		const newEdit = linkEdits?.slice();
		const editIndex =
			newEdit?.findIndex((linkToEdit) => property === linkToEdit.value) ?? -1;
		...
		<>
			...
			{isPremium ? (
				<Paragraph style={{ marginBottom: '32px' }}>
					Note: A Twitch link is required to enable live badges on your avatar
					photo and popular clips, recent channels, and channel schedule
					sections on your profile page.
				</Paragraph>
			) : null}
			<Paragraph>
				Feel free to paste in the whole link. We will trim it up for you. Yes,
				we know, we love you, too.
			</Paragraph>
			{modal.type === 'Add Link' && (
				<AddLink linkEdits={linkEdits} setLinkEdits={setLinkEdits} />
			)}
			{linkEdits?.map((link) => {
			...
			<ButtonFlex>
				...
				<div style={{ flex: 0.5, textAlign: 'center' }}>
					<Button
						...
						onClick={async () =>
							mutate({ channelId: data?.id!, links: linkEdits! })
						}
						style={{
							margin: '0 auto',
						}}
