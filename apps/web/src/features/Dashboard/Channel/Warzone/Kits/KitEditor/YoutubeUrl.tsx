...
function YoutubeId({ ...props }) {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value.includes('m.youtube.com')) {
      return dispatch(
        updateYoutubeURL(e.target.value.split('v=').slice(-1)[0]),
      );
    }

    return dispatch(
      updateYoutubeURL(
        trimPrefix('https://www.youtube.com/watch?v=', e.target.value),
      ),
    );
  };
  ...
  return (
    <Styled.Container>
      ...
      <Styled.ParagraphHelper>
        Enter the link to your YouTube video below. We HIGHLY recommend that the
        video be about either this kit specifically or a general "loadouts"
        video.
      </Styled.ParagraphHelper>
      <Styled.ParagraphHelper>
        Tip: You can adjust autoplay in your account settings.
      </Styled.ParagraphHelper>
      <TextInput
