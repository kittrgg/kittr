...
function Preview({ overlay }: Props) {
	...
	const hasAKitSelected =
		Object.keys(overlay?.primaryWzTwoKit || {}).length > 0 ||
		Object.keys(overlay?.secondaryWzTwoKit || {}).length > 0;
		...
		<>
			...
			<PreviewContainer ref={observe}>
				...
				<H3 style={{ marginTop: 0, marginBottom: '24px' }}>
					{overlayOptions[cursor].style.toUpperCase()}
				</H3>
				{hasAKitSelected && data?.id ? (
					<Overlay
						id={data.id}
						overlayStyle={overlayOptions[cursor].style as ActiveKitOverlays}
						previewWidth={width}
					/>
				) : (
				...
				<Styled.Paragraph style={{ marginTop: '24px' }}>
					Set your OBS browser source to {overlayOptions[cursor].width} x{' '}
					{overlayOptions[cursor].height} px.
				</Styled.Paragraph>
				{cursor < overlayOptions.length - 1 && (
					<SVG.Arrow
						onClick={() =>
							setCursor((cursor) =>
								cursor < overlayOptions.length - 1 ? cursor + 1 : cursor,
							)
						}
						style={{
							transform: 'rotate(90deg)',
