	...
	return (
		<>
			...
			>
				...
				<Title>{`${baseName}${
					matchedBase.length === 1 && firstMatchedBaseUserTitle.length > 3
						? firstMatchedBaseUserTitle
						: ''
				}`}</Title>
				{/* BRANDON THE WAMPUS WAS HERE */}
				{matchedBase.length > 1 && (
					<KitCount isActive={weaponQuery === baseNameCleanse}>
						({matchedBase.length})
					</KitCount>
				)}
			</Container>
			...
			>
				<HeightAnimate ref={childRef}>
					{weaponQuery === baseNameCleanse &&
						...
						matchedBase
							.sort((a, b) =>
								sortAlphabetical(a.customTitle || '', b.customTitle || ''),
							)
...
const Container = styled.button<{ isActive: boolean }>`
  ...
  background-color: ${(props) =>
		props.isActive ? colors.lightest : colors.light};
  color: ${colors.white};
