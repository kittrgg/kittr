...
function Attachments() {
  ...
  return (
    ...
    >
      {options.length === 0 && (
        <Styled.KitOptionLabel>No attachments assigned!</Styled.KitOptionLabel>
      )}
              ...
              <Fragment key={slot.slotKey}>
                ...
                {tunes !== undefined && (
                  <Grid style={{ width: '80%', marginLeft: '4%' }}>
                    {tunes[
                      tunes.findIndex((tune) => tune.kitOptionId === slot.id)
                    ]?.horz ? (
                      <Grid.Col span={6}>
...
export const Container = styled.div<{
  ...
  width: 100%;
  // height: ${(props) =>
    Math.max((props.numOfOptions + props.numOfTunes / 4) * 100, 80)}px;
  padding: 2em 8%;
  ...
  @media (max-width: 650px) {
    // height: ${(props) =>
      Math.max((props.numOfOptions + props.numOfTunes / 1.25) * 75, 80)}px;
  }
`;
