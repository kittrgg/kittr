...
export const useActiveChannelKit = () => {
  ...
  const kitName = query.k as string | undefined;
  // Warzone 2 Kits
  const activeWz2Weapon =
    channelData?.warzoneTwoKits.find(
      (kit) =>
        (
          kit.base.displayName.replace(' ', '-') +
          kit.customTitle?.replace(' ', '-')
        ).replace(' ', '-') === kitName?.replace(' ', '-'),
    ) ??
    channelData?.warzoneTwoKits.find(
      (kit) =>
        kit.base.displayName.replace(' ', '-') === kitName?.replace(' ', '-'),
    ) ??
