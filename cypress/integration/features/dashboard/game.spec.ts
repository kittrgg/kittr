...
describe("Games & Kits Actions", () => {
	...
	beforeEach(() => {
		cy.intercept("GET", "https://kittr-test.herokuapp.com/socket.io/**").as("socket")
		cy.intercept("https://www.googleapis.com/identitytoolkit/**", (req) => req.reply("")).as("auth mocks only")
		cy.intercept("/api/manager/getInfo?uid=*", (req) => req.reply(managers[req.query.uid])).as("getManagerInfo")
	...
	it("Creates first kit", () => {
		...
		cy.get("[name=channelQuote]").type("totally just a test quote to make sure this works.")
	...
	it("Quick exports command", () => {
		...
		cy.contains(`!addcom !em2 localhost:3000/c/${channel.urlSafeName}/warzone/?k=EM2`)
	})
	...
	it("Exports bot commands", () => {
		...
		cy.get("[data-cy=user-toggle] > [data-cy=toggler-value-1]").click()
		cy.contains(`!editcom !loadout localhost:3000/c/${channel.urlSafeName}/warzone`)
		...
		cy.contains(`!editcom !loadout localhost:3000/c/${channel.urlSafeName}/warzone is the best`)
