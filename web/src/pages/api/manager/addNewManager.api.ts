...
handler.post(async (req: NextApiRequest, res: NextApiResponse) => {
	...
	if (userRole === "Administrator" || userRole === "Owner") {
		try {
				...
				try {
					...
					if (!data) {
						return res.status(409).json({
							error: true,
							message: "This manager already has a role."
						})
					}
					...
					return res.status(500).json({
						error: true,
						message: "We messed up. Please let us know that there is a problem. (Error code: 3457)"
					})
				}
			}
		} catch (error) {
			...
			if (isFirebaseError(error)) {
				if (error.code === "auth/invalid-email") {
					return res.status(404).json({
						error: true,
						message: "That's an invalid email."
					})
				}
				...
				if (error.code === "auth/user-not-found") {
					return res.status(404).json({
						error: true,
						message: "Username doesn’t match our records. Be sure your new manager has already created a kittr account."
					})
				}
			}
			...
			return res.status(500).json({
				error: true,
				message: "We messed up. Please let us know that there is a problem. (Error code: 3458)"
			})
		}
	} else {
