...
describe("Channel Settings", () => {
	beforeEach(() => {
		cy.intercept("https://www.googleapis.com/identitytoolkit/**", (req) => req.reply("")).as("auth mocks")
		cy.logout()
	...
	it("Shows all managers", () => {
		...
		cy.get("[data-cy=manager] > p").filter(':contains("Owner")').should("have.length", 1)
		cy.get("[data-cy=manager] > p").filter(':contains("Editor")').should("have.length", 2)
	})
	...
	it("Adjusts YouTube autoplay", () => {
		...
		cy.get("[data-cy=toggler-value-1]").should("have.css", "background-color", "rgb(155, 155, 155)")
		cy.get("[data-cy=toggler-value-0]").click()
		cy.get("[data-cy=toggler-value-0]").should("have.css", "background-color", "rgb(155, 155, 155)")
	})
